...
def draft_keyboard(draft_id: int | str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="✂️ Короче", callback_data=f"short:{draft_id}"), InlineKeyboardButton(text="🎬 Shorts Studio", callback_data=f"shortsv2:start:{draft_id}")],
...
def main_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡ Создать", callback_data="panel:generate"), InlineKeyboardButton(text="🧠 Doctor", callback_data="v2:doctor")],
        [InlineKeyboardButton(text="📅 Календарь", callback_data="panel:scheduled"), InlineKeyboardButton(text="📊 Growth", callback_data="panel:analytics")],
        [InlineKeyboardButton(text="🎬 Studio", callback_data="panel:shorts"), InlineKeyboardButton(text="🛒 Sales", callback_data="panel:orders")],
        [InlineKeyboardButton(text="⚽ Football", callback_data="panel:football"), InlineKeyboardButton(text="🎁 Gifts", callback_data="panel:gifts")],
        [InlineKeyboardButton(text="🧠 Knowledge", callback_data="panel:courses"), InlineKeyboardButton(text="⚙️ System", callback_data="panel:system")],
    ])


def doctor_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔬 Проверить другой", callback_data="v2:doctor")],
        [InlineKeyboardButton(text="⚡ Создать новый", callback_data="panel:generate"), InlineKeyboardButton(text="🛒 Решение под ключ", callback_data="panel:orders")],
    ])


def install(legacy):
    """Install isolated v2 features over the stable legacy runtime."""
    ...
    @router.callback_query(F.data == "v2:doctor")
    async def doctor_start(c: CallbackQuery, state: FSMContext):
        if not legacy.admin(c): return
        await state.set_state(DoctorFlow.waiting_text)
        await c.answer()
        await c.message.answer(
            "🧠 <b>Content Doctor</b>\n\nПришли текст поста, оффера или сценария одним сообщением.\nЯ разложу его по HOOK / CLARITY / EMOTION / TRUST / CTA / OFFER.\n\nБез выдуманных прогнозов охватов — только понятный редакторский разбор.",
            parse_mode=ParseMode.HTML,
        )

    @router.message(DoctorFlow.waiting_text, F.text)
    async def doctor_text(message: Message, state: FSMContext):
        if not legacy.admin(message): return
        report = diagnose(message.text or "")
        await state.clear()
        await message.answer(render_doctor(report), parse_mode=ParseMode.HTML, reply_markup=doctor_keyboard())

    async def show_review(c: CallbackQuery, job_id: int | str, *, new_message: bool = False):
        brief = studio.sessions.load(job_id)
        if brief is None: return await c.answer("Shorts-сессия не найдена", show_alert=True)
        text = brief_text(brief)
        if not new_message and getattr(c.message, "text", None):
            try:
                await legacy.safe_edit_text(c.message, text, parse_mode=ParseMode.HTML, reply_markup=review_keyboard(job_id)); return
    ...
    async def render_job(c: CallbackQuery, job_id: int | str):
        if not studio.renderer.ready:
            return await c.message.answer("✅ <b>Сценарий подтверждён</b>\n\nShorts Worker пока не подключён к этой версии. Монтаж не запускал — сценарий сохранён.", parse_mode=ParseMode.HTML)
        status = await c.message.answer("🎬 <b>Собираю Shorts · 0%</b>\n\nСейчас: подготовка", parse_mode=ParseMode.HTML)
        async def progress(value: int):
            phase = "озвучка" if value < 20 else "кадры" if value < 75 else "субтитры и финальный рендер"
            try: await status.edit_text(f"🎬 <b>Собираю Shorts · {value}%</b>\n\nСейчас: {phase}", parse_mode=ParseMode.HTML)
            except Exception: pass
    ...
    async def start_short(c: CallbackQuery):
        ...
        try: job_id,brief=await studio.start(draft)
        except Exception as exc:
            log.exception("Shorts v2 script generation failed"); return await c.message.answer(f"❌ Сценарий не собрался: {html.escape(str(exc)[:300])}",parse_mode=ParseMode.HTML)
    ...
    async def review_short(c: CallbackQuery):
        if not legacy.admin(c): return
        await c.answer(); await show_review(c,c.data.rsplit(":",1)[-1])
    ...
    async def rewrite_short(c: CallbackQuery):
        ...
        _,_,mode,job_id=parts; await c.answer("Переписываю только сценарий…")
        try: await studio.rewrite(job_id,mode)
    ...
    async def styles(c: CallbackQuery):
        ...
        job_id=c.data.rsplit(":",1)[-1]; brief=studio.sessions.load(job_id)
        if not brief: return await c.answer("Shorts-сессия не найдена",show_alert=True)
        await c.answer(); await c.message.answer("🎨 <b>Подача</b>\n\nСтиль меняет сам сценарий, поэтому после выбора текст будет пересобран до монтажа.",parse_mode=ParseMode.HTML,reply_markup=style_keyboard(job_id,brief.delivery_preset))
    ...
    async def choose_style(c: CallbackQuery):
        ...
        _,_,preset,job_id=parts; await c.answer("Пересобираю сценарий под стиль…")
        try: brief=await studio.restyle(job_id,preset)
        except Exception as exc: return await c.message.answer(f"❌ {html.escape(str(exc)[:300])}",parse_mode=ParseMode.HTML)
        await c.message.answer(brief_text(brief),parse_mode=ParseMode.HTML,reply_markup=review_keyboard(job_id))
    ...
    async def subtitles(c: CallbackQuery):
        ...
        job_id=c.data.rsplit(":",1)[-1]; brief=studio.sessions.load(job_id)
        if not brief: return await c.answer("Shorts-сессия не найдена",show_alert=True)
        await c.answer(); await c.message.answer("💬 <b>Субтитры</b>\n\nPunch — самый динамичный. Clean — спокойнее. Sport и Meme дают более выраженный характер.",parse_mode=ParseMode.HTML,reply_markup=subtitle_keyboard(job_id,brief.subtitle_preset))
    ...
    async def choose_subtitle(c: CallbackQuery):
        ...
        parts=c.data.split(":")
        if len(parts)!=4: return await c.answer("Некорректный стиль",show_alert=True)
        _,_,preset,job_id=parts; brief=studio.choose_subtitle(job_id,preset); await c.answer("Субтитры выбраны")
        if brief.approved: return await render_job(c,job_id)
        await c.message.answer(brief_text(brief),parse_mode=ParseMode.HTML,reply_markup=review_keyboard(job_id))

    @router.callback_query(F.data.startswith("shortsv2:scenes:"))
    async def remix_scenes(c: CallbackQuery):
        if not legacy.admin(c): return
        job_id=c.data.rsplit(":",1)[-1]; current=studio.sessions.load(job_id)
        if not current: return await c.answer("Shorts-сессия не найдена",show_alert=True)
        was_approved=current.approved; await c.answer("Подбираю другие кадры…")
        try: await studio.remix_scenes(job_id)
        except Exception as exc: return await c.message.answer(f"❌ {html.escape(str(exc)[:300])}",parse_mode=ParseMode.HTML)
        if was_approved: return await render_job(c,job_id)
        await show_review(c,job_id,new_message=not bool(getattr(c.message,"text",None)))
    ...
    async def approve_short(c: CallbackQuery):
        ...
        try: studio.approve(job_id)
        except Exception as exc: return await c.answer(str(exc)[:150],show_alert=True)
        await c.answer("Сценарий принят"); await render_job(c,job_id)

    @router.callback_query(F.data.startswith("shortsv2:back:"))
    async def back_to_post(c: CallbackQuery):
        if not legacy.admin(c): return
        job_id=c.data.rsplit(":",1)[-1]; brief=studio.sessions.load(job_id)
        if not brief or brief.draft_id is None: return await c.answer("Исходный пост не найден",show_alert=True)
        await c.answer(); await c.message.answer(f"↩️ <b>Пост #{brief.draft_id}</b>",parse_mode=ParseMode.HTML,reply_markup=draft_keyboard(brief.draft_id))
