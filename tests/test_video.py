...
def test_busy_worker_gateway_error_is_transient():
    assert VideoFactory.transient_status_error(RuntimeError('Shorts Worker HTTP 502: Application failed to respond'))
    assert not VideoFactory.transient_status_error(RuntimeError('Shorts Worker HTTP 401: invalid key'))
    assert not VideoFactory.transient_status_error(RuntimeError('Shorts Worker HTTP 404: task not found'))
